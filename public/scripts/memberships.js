...
function loadHTMLTable(data) {
    ...
    data.forEach(function ({group_id, group_name, date_joined, group_num, group_online, group_url}) {
        ...
        tableHtml += `<td><a href= ${group_url}> ${group_name} </a></td>`; //  tableHtml += `<td>${group_name}</td>`;
...
//     }
//     let tableHtml = "";
//     data.forEach(function ({group_id, group_name, date_joined, group_num, group_online}) {
//         tableHtml += "<tr>";
//         tableHtml += `<td>${group_id}</td>`;
//         tableHtml += `<td><a href= "home.html"> ${group_name} </a></td>`; //  tableHtml += `<td>${group_name}</td>`;
//         tableHtml += `<td>${new Date(date_joined).toLocaleString()}</td>`;
//         tableHtml += `<td>${group_num}</td>`;
//         tableHtml += `<td>${group_online}</td>`;
//         tableHtml += `<td><button class="navigate-group-btn" data-id=${group_id}>Navigate</td>`;
...
function LeaveGroup(group_id) { 
    fetch('http://localhost:3000/delete/' + group_id, {
        method: 'DELETE'
    })
    .then(response => response.json())
    .then(data => {
...
module.exports = {loadHTMLTable :loadHTMLTable};